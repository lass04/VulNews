...
export const fetchAndStoreLatestNvdCVEs = async () => {
  ...
  try {
    ...
    console.log(`✅ NVD Cron done | Inserted: ${inserted} | Updated: ${updated}`);
  } catch (error) {
...
export const getLatestNvd = async (req,res) => {
  ...
  try{
    ...
    const nvds = await Cve.find({source:"NVD"}).skip((page-1)*limit).limit(limit);
